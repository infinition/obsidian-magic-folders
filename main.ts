...
type SortType = 'name' | 'modified' | 'created' | 'type' | 'size' | 'tags' | 'tag';
type LogicOperator = 'OR' | 'AND' | 'NOT';

type LanguageCode = 'en' | 'fr' | 'es' | 'de' | 'it';

interface CachedFileInfo {
	path: string;
	basename: string;
	extension: string;
	matchedFilters: MagicFilter[];
}

interface MagicFolderCache {
	folderId: string;
...
interface FileWithMatchedFilters {
	file: TFile;
	matchedFilters: MagicFilter[];
}

const DEFAULT_SETTINGS: MagicFoldersSettings = {
	magicFolders: [],
	showInExplorer: true,
...
const AVAILABLE_ICONS = [
	'folder-search', 'wand-2', 'sparkles', 'star', 'heart', 'bookmark',
	'tag', 'hash', 'filter', 'layers', 'box', 'archive', 'briefcase',
	'folder-heart', 'folder-key', 'folder-check', 'folder-clock',
	'zap', 'flame', 'target', 'trophy', 'crown', 'gem', 'rocket',
	'lightbulb', 'brain', 'puzzle', 'compass', 'map', 'globe'
];

const PRESET_COLORS = [
	'#e74c3c', '#e67e22', '#f1c40f', '#2ecc71', '#1abc9c',
	'#3498db', '#9b59b6', '#e91e63', '#00bcd4', '#607d8b',
	'#795548', '#ff5722', '#8bc34a', '#673ab7', '#ff9800'
];
...
const TRANSLATIONS: Record<LanguageCode, Record<string, string>> = {
	...
	fr: {
		...
		settings_show_tooltips_desc: 'Afficher les tags et filtres matchés au survol des fichiers.',
		settings_language_name: 'Langue',
		settings_language_desc: 'Choisir la langue du plugin.',
		settings_custom_icons_name: 'Utiliser des icônes personnalisées',
		settings_custom_icons_desc: 'Activer des icônes depuis un dossier du coffre.',
		settings_custom_icons_path_name: 'Dossier d’icônes',
		settings_custom_icons_path_desc: 'Chemin du dossier (coffre ou externe) contenant des icônes (svg/png/jpg).',
		settings_show_new_badge_name: 'Badge nouveaux éléments',
		settings_show_new_badge_desc: 'Afficher un badge sur les dossiers repliés quand il y a du nouveau.',
		settings_unread_highlight_name: 'Mettre en évidence les fichiers non lus',
		settings_unread_highlight_desc: 'Met en gras les nouveaux fichiers jusqu’à leur ouverture.',
		settings_show_file_count_name: 'Afficher le nombre de fichiers',
		settings_show_file_count_desc: 'Afficher le compteur de fichiers à côté du nom du dossier.',
		settings_refresh_on_change_name: 'Actualisation auto',
		settings_refresh_on_change_desc: 'Rafraîchir les dossiers quand les fichiers changent.',

		menu_create_here: 'Créer un Magic Folder ici',
		...
		menu_sort_modified: 'Trier par : Modifié',
		menu_sort_created: 'Trier par : Date de création',
		...
		command_create: 'Créer un Magic Folder',
		command_create_here: 'Créer un Magic Folder dans le dossier courant',
		command_refresh: 'Rafraîchir tous les Magic Folders',

		notice_created: 'Magic Folder "{name}" créé !',
		notice_updated: 'Magic Folder "{name}" mis à jour !',
		notice_deleted: 'Magic Folder "{name}" supprimé !',
		notice_refresh: '{name} rafraîchi !',
		notice_name_required: 'Veuillez saisir un nom pour votre Magic Folder',
		notice_marked_all_read: 'Tous les fichiers de "{name}" marqués comme lus.',
		notice_marked_all_unread: 'Tous les fichiers de "{name}" marqués comme non lus.',

		modal_create_title: 'Créer un Magic Folder',
		modal_edit_title: 'Éditer un Magic Folder',
		...
		modal_icon_desc: 'Choisissez une icône pour votre Magic Folder',
		...
		do_magic_help_frontmatter: 'Frontmatter : clé:valeur',
		do_magic_help_multi: 'Plusieurs éléments séparés par des espaces',
	...
	es: {
		...
		settings_custom_icons_path_desc: 'Ruta de carpeta (bóveda o externa) con iconos (svg/png/jpg).',
		...
		settings_unread_highlight_name: 'Resaltar archivos no leídos',
		...
		settings_show_file_count_desc: 'Muestra el número de archivos junto al nombre de la carpeta.',
		settings_refresh_on_change_name: 'Actualización automática',
		settings_refresh_on_change_desc: 'Actualizar cuando cambien los archivos.',

		menu_create_here: 'Crear Magic Folder aquí',
		...
		menu_sort_created: 'Ordenar por: Fecha de creación',
		menu_sort_type: 'Ordenar por: Tipo de archivo',
		menu_sort_size: 'Ordenar por: Tamaño',
		...
		notice_marked_all_read: 'Todos los archivos en "{name}" marcados como leídos.',
		notice_marked_all_unread: 'Todos los archivos en "{name}" marcados como no leídos.',
		...
		modal_sort_by_desc: 'Cómo ordenar los archivos en este folder',
		modal_sort_name: 'Nombre (A-Z)',
		modal_sort_modified: 'Última modificación',
		modal_sort_created: 'Fecha de creación',
		modal_sort_type: 'Tipo de archivo',
		modal_sort_size: 'Tamaño de archivo',
		...
		do_magic_help_ext: 'Extensión: .pdf o ext:pdf',
	...
	de: {
		...
		settings_language_desc: 'Plugin-Sprache auswählen.',
		...
		settings_show_new_badge_name: 'Badge für neue Elemente',
		...
		settings_unread_highlight_desc: 'Neue Dateien fett anzeigen bis sie geöffnet werden.',
		...
		settings_refresh_on_change_desc: 'Aktualisieren, wenn Dateien sich ändern.',
		...
		notice_deleted: 'Magic Folder "{name}" gelöscht!',
		notice_refresh: '{name} aktualisiert!',
		notice_name_required: 'Bitte einen Namen für deinen Magic Folder eingeben',
		...
		modal_color_desc: 'Wähle eine Farbe für deinen Magic Folder',
		modal_icon_label: 'Icon',
		modal_icon_desc: 'Wähle ein Icon für deinen Magic Folder',
		modal_emoji_label: 'Emoji (optional)',
		modal_emoji_desc: 'Wähle ein Emoji für deinen Magic Folder',
		...
		modal_sort_name: 'Name (A-Z)',
		modal_sort_modified: 'Zuletzt geändert',
		modal_sort_created: 'Erstellungsdatum',
		modal_sort_type: 'Dateityp',
		modal_sort_size: 'Dateigröße',
		modal_sort_tag: 'Tag',
		...
		do_magic_logic_label: 'Hinzufügen als: ',
		do_magic_input_placeholder: 'Tippe # für Tags, [[ für Links, . für Endungen...',
		do_magic_add: 'Hinzufügen',
	...
	it: {
		...
		settings_unread_highlight_desc: 'Metti in grassetto i nuovi file finché non vengono aperti.',
		...
		do_magic_help_multi: 'Più token separati da spazi',
...
class MagicFoldersSettingTab extends PluginSettingTab {
	plugin: MagicFoldersPlugin;

	constructor(app: any, plugin: MagicFoldersPlugin) {
		super(app, plugin);
		this.plugin = plugin;
	}
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName(t('settings_show_tooltips_name'))
			.setDesc(t('settings_show_tooltips_desc'))
			.addToggle(toggle => {
				toggle.setValue(this.plugin.settings.showTooltips)
		...
		new Setting(containerEl)
			.setName(t('settings_refresh_on_change_name'))
			.setDesc(t('settings_refresh_on_change_desc'))
			.addToggle(toggle => {
				toggle.setValue(this.plugin.settings.refreshOnChange)
					.onChange(async (value) => {
		...
		new Setting(containerEl)
			.setName(t('settings_show_new_badge_name'))
			.setDesc(t('settings_show_new_badge_desc'))
			.addToggle(toggle => {
				toggle.setValue(this.plugin.settings.showNewItemsBadge)
					.onChange(async (value) => {
		...
		new Setting(containerEl)
			.setName(t('settings_unread_highlight_name'))
			.setDesc(t('settings_unread_highlight_desc'))
			.addToggle(toggle => {
				toggle.setValue(this.plugin.settings.highlightUnreadFiles)
					.onChange(async (value) => {
		...
		new Setting(containerEl)
			.setName(t('settings_show_file_count_name'))
			.setDesc(t('settings_show_file_count_desc'))
			.addToggle(toggle => {
				toggle.setValue(this.plugin.settings.showFileCount)
					.onChange(async (value) => {
		...
		new Setting(containerEl)
			.setName(t('settings_custom_icons_name'))
			.setDesc(t('settings_custom_icons_desc'))
			.addToggle(toggle => {
				toggle.setValue(this.plugin.settings.customIconsEnabled)
					.onChange(async (value) => {
		...
		const iconPathSetting = new Setting(containerEl)
			.setName(t('settings_custom_icons_path_name'))
			.setDesc(t('settings_custom_icons_path_desc'));
		iconPathSetting.addText(text => {
			text.setPlaceholder('Assets/Icons')
...
export default class MagicFoldersPlugin extends Plugin {
	settings: MagicFoldersSettings;
	virtualElements: HTMLElement[] = [];
	styleEl: HTMLStyleElement | null = null;
	fileExplorerButton: HTMLElement | null = null;
	refreshTimeout: NodeJS.Timeout | null = null;
	matchedFiltersCache: Map<string, Map<string, MagicFilter[]>> = new Map();
	mutationObserver: MutationObserver | null = null;
	isRendering = false;
	isInitialLoad = true;
	tooltipElements: Set<HTMLElement> = new Set();
	tooltipTimers: Map<HTMLElement, { show: number | null, hide: number | null }> = new Map();
	draggingMagicFolderId: string | null = null;
	...
	t(key: string, vars?: Record<string, any>) {
		...
		if (vars) {
			out = out.replace(/\{(\w+)\}/g, (_, k) => String(vars[k] ?? `{${k}}`));
		}
		return out;
	}
	...
	setupFileExplorerObserver() {
		...
		this.mutationObserver = new MutationObserver((mutations) => {
			...
			const missingMagicFolderIds = new Set();
	...
	insertCreateButton() {
		...
		(0, setIcon)(this.fileExplorerButton, "wand-2");
	...
	createMagicFolderPlaceholder(magicFolder) {
		...
		if (magicFolder.customIconPath) {
			...
			if (resourcePath) {
				...
				(0, setIcon)(magicIcon, magicFolder.icon);
				// Apply transforms to standard icons too
	...
	createMagicFolderDOMFromCache(magicFolder, cachedFiles) {
		...
		(0, setIcon)(collapseIcon, "right-triangle");
		...
		const unreadSet = this.settings.highlightUnreadFiles && Array.isArray(magicFolder.unreadFilePaths)
			? new Set(magicFolder.unreadFilePaths)
			: null;
	...
	updateIconTypeInDom(folderId, type, value) {
		...
		// value: icon name, emoji char, or image path/dataUrl
		const folderEl = document.querySelector(`[data-magic-folder-id="${folderId}"]`);
		if (!folderEl) return;
	...
	setCachedIconToLocalStorage(iconPath, resource) {
		try {
			if (!resource || resource.length > 900000) return;
			const key = this.getLocalStorageCacheKey(iconPath);
	...
	async saveCustomIconForFolder(folderId, sourcePath, size, keepRatio) {
		try {
			...
			if (ext === ".gif") {
				const filePath = this.getCustomIconStoragePath(folderId, ".gif");
				...
				const previewDataUrl = await this.readFileAsDataUrl(filePath);
				if (previewDataUrl) this.setCustomIconCache(filePath, previewDataUrl);
				return { filePath, previewDataUrl: previewDataUrl || "" };
			}
	...
	getCustomIconResource(iconPath) {
		...
		if (this.isExternalPath(iconPath)) {
			try {
				...
				if (mime === "image/svg+xml") {
					const svgText = fs.readFileSync(iconPath, "utf8");
					const resource = `data:${mime};utf8,${encodeURIComponent(svgText)}`;
					this.customIconResourceCache.set(iconPath, resource);
					return resource;
				}
				const buffer = fs.readFileSync(iconPath);
	...
	createMagicFolderDOM(magicFolder, filesWithFilters) {
		...
		if (magicFolder.customIconPath) {
			...
			if (resourcePath) {
				...
				(0, setIcon)(magicIcon, magicFolder.icon);
				// Apply transforms to standard icons
				const svg = magicIcon.querySelector("svg");
				if (svg) {
					svg.style.width = `${size}px`;
					svg.style.height = `${size}px`;
	...
	showMagicFolderContextMenu(e, magicFolder) {
		...
		menu.addItem((item) => {
			item.setTitle(this.t('menu_refresh')).setIcon("refresh-cw").onClick(() => {
				this.renderAllMagicFolders();
				new Notice(this.t('notice_refresh', { name: magicFolder.name }));
			});
		});
		// --- NOUVELLES OPTIONS: MARK ALL AS READ / UNREAD ---
		menu.addItem((item) => {
			item.setTitle(this.t('menu_mark_all_read'))
				.setIcon("check-circle-2")
				.onClick(async () => {
					magicFolder.unreadFilePaths = [];
					await this.saveSettings();
					this.renderAllMagicFolders();
					new Notice(this.t('notice_marked_all_read', { name: magicFolder.name }));
				});
		});
		menu.addItem((item) => {
			item.setTitle(this.t('menu_mark_all_unread'))
				.setIcon("circle")
				.onClick(async () => {
					const filesWithFilters = await this.getMatchingFilesWithFilters(magicFolder);
					...
					await this.saveSettings();
					this.renderAllMagicFolders();
					new Notice(this.t('notice_marked_all_unread', { name: magicFolder.name }));
				});
		});
	...
	async createMagicFolder(folder) {
		...
		await this.saveSettings();
		await this.renderAllMagicFolders();
		new Notice(this.t('notice_created', { name: folder.name }));
	}
	async updateMagicFolder(folder) {
		...
		if (index !== -1) {
			...
			await this.saveSettings();
			await this.renderAllMagicFolders();
			new Notice(this.t('notice_updated', { name: folder.name }));
		}
	}
	async deleteMagicFolder(id) {
		...
		await this.saveSettings();
		await this.renderAllMagicFolders();
		new Notice(`Magic Folder "${folder == null ? void 0 : folder.name}" deleted!`);
	}
	getAllFolders() {
		const folders = [""];
		const allFolders = this.app.vault.getAllLoadedFiles().filter((f) => f instanceof TFolder).map((f) => f.path).sort();
...
class MagicFolderModal extends Modal {
	...
	constructor(app, plugin, existingFolder) {
		...
		this.folder = existingFolder ? {
			...
			customIconOffsetX: existingFolder.customIconOffsetX ?? 0,
	...
	onOpen() {
		...
		// --- NAME ---
		new Setting(contentEl).setName(t('modal_name_label')).setDesc(t('modal_name_desc')).addText((text) => {
		...
		// --- HIDE NAME (ICON ONLY) ---
		new Setting(contentEl).setName("Hide name (Icon only)").setDesc("Show only the icon in the sidebar.").addToggle((toggle) => {
		...
		// --- ICONS ---
		const iconSetting = new Setting(contentEl).setName(t('modal_icon_label')).setDesc(t('modal_icon_desc'));
		...
		// --- GENERIC ICON ADJUSTMENTS (Moved outside 'customIconsEnabled') ---
		const iconAdjustSetting = new Setting(contentEl).setName("Icon Adjustments");
		...
		const updateStyles = () => {
			...
			// Update DOM (File Explorer)
			if (!this.isNew) {
				// Update live in memory settings for immediate preview context if needed
				const liveFolder = this.plugin.settings.magicFolders.find((f) => f.id === this.folder.id);
				if (liveFolder) {
		...
		if (this.plugin.settings.customIconsEnabled && this.plugin.settings.customIconFolder) {
			const customIconSetting = new Setting(contentEl).setName(t('modal_custom_icons_label'));
		...
		// --- SORT BY ---
		new Setting(contentEl).setName(t('modal_sort_by_label')).setDesc(t('modal_sort_by_desc')).addDropdown((dropdown) => {
...
class DoMagicModal extends Modal {
	plugin: MagicFoldersPlugin;
	folder: MagicFolder;
	filterInput: HTMLInputElement | null = null;
	filtersContainer: HTMLElement | null = null;
	previewContainer: HTMLElement | null = null;
	suggestionsContainer: HTMLElement | null = null;
	allTags: string[] = [];
	allLinks: string[] = [];
	selectedSuggestionIndex = -1;
	currentLogic: LogicOperator = 'OR';
	private domEventCleanup: Array<() => void> = [];

	private addDomEvent<K extends keyof HTMLElementEventMap>(
		el: HTMLElement,
		type: K,
	...
	constructor(app: any, plugin: MagicFoldersPlugin, folder: MagicFolder) {
		super(app);
		this.plugin = plugin;
	...
	onInputChange() {
		...
		const suggestions: { text: string; type: 'tag' | 'link' | 'extension' }[] = [];
	...
	renderSuggestions(suggestions: { text: string; type: 'tag' | 'link' | 'extension' }[]) {
		...
		for (let i = 0; i < suggestions.length; i++) {
			...
			item.addEventListener('mouseenter', () => {
				this.suggestionsContainer?.querySelectorAll('.magic-suggestion-item').forEach(el => {
	...
	selectSuggestion(suggestion: { text: string; type: 'tag' | 'link' | 'extension' }) {
		let filter: MagicFilter;
	...
	parseFilterInput(input: string): MagicFilter[] {
		const filters: MagicFilter[] = [];
	...
	renderFilters() {
		...
		const logicGroups: Record<LogicOperator, MagicFilter[]> = {
			'OR': [],
			'AND': [],
			'NOT': []
		};
		...
		const logicColors: Record<LogicOperator, string> = {
			'OR': '#2ecc71',
			'AND': '#3498db',
			'NOT': '#e74c3c'
		};
		...
		for (const logic of ['OR', 'AND', 'NOT'] as LogicOperator[]) {
